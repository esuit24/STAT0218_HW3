...
library(tidyverse)
library(rattle)
library(rpart) # for a single classification tree
...
# create a variable "night" that puts 1 if an observation happened between 7 pm and 6 am and 0 otherwise
...
```{r}
bag_var_imp <- as.data.frame(bag_veh_rf$importance) # create a dataframe for importance
...
Thus, "night" has the smallest (least positive) mean decrease accuracy and "policePrecinct" has the smallest (least positive) mean decrease Gini. 

## Remove the least important predictor variables found above, build a new bagged model and find the overall OOB error

We remove "night" and "policePrecinct" and examine if we can build a bagged model with a lower OOB error. 
...
Removing "night" and "policePrecinct" increased the OOB error rate slightly.
