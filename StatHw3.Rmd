...
tree <- rpart(vehicleSearch ~ race + preRace + gender + policePrecinct + reason + problem + case + lat + long, data = stop_data_factored)
...
```{r}
table(stop_data_factored$vehicleSearch)
```
...
after_gf_revised <- after_gf %>%
  filter(!(problem == 'Truancy (P)')) %>%
  mutate(problem = droplevels(problem))

rf2 <- randomForest(problem ~ gender + preRace, 
